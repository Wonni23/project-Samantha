      ...
      children: [
        ...
        Row(
          ...
          children: [
            // 녹음 테스트 버튼
            _TestButton(
              icon: audioState.isRecording ? Icons.stop : Icons.fiber_manual_record,
              label: audioState.isRecording ? '중지' : '녹음',
              color: audioState.isRecording ? Colors.redAccent : AppColors.primary,
              onPressed: () async {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(audioState.isRecording ? '녹음 중지 시도' : '녹음 시작 시도')),
                );
                if (audioState.isRecording) {
                  await audioNotifier.stopRecording();
                } else {
            ...
            _TestButton(
              ...
              label: audioState.isPlaying ? '재생 중' : '재생',
              color: audioState.isPlaying ? AppColors.accentSage : AppColors.accentBrown,
              onPressed: () async {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('재생 버튼 클릭됨')),
                );
                if (audioState.isPlaying) {
                  await audioNotifier.stopPlaying();
                } else {
                  await audioNotifier.startPlaying();
                }
              },
            ),
          ],
